...
Future<List> getPostulacionesPorChaza(String idChaza) async { //matriz 3D con listas de listas de mapas
  ...
  CollectionReference collectionReferencePostulaciones = db.collection('Postulaciones');
  QuerySnapshot querypostulaciones = await collectionReferencePostulaciones
      .where('IDChaza', isEqualTo: idChaza)
      .get();
  querypostulaciones.docs.forEach((doc){
    dynamic Trabajador = doc.get('IDTrabajador');
    trabajadores.add(Trabajador);  //Mete en una lista los ids de todos los postulados
  ...
  for(int i = 0; i<trabajadores.length; i++ ){
    DocumentSnapshot<Map<String, dynamic>> infotrabajador = await db
        .collection('Trabajador').doc(trabajadores[i].toString().trim()).get();
    datosTrabajadorHorario.add(infotrabajador.data());   // Mete en la lista los datos de cada id
    DocumentSnapshot<Map<String, dynamic>> infohorario = await db
        .collection('Horario').doc(horarios[i].toString().trim()).get();
    datosTrabajadorHorario.add(infohorario.data());      //Mete en la misma lista los detalles de cada horario
    resultadosTotales.add(datosTrabajadorHorario);        //Mete pareja Trabajador-horario en la lista final
